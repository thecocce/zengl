...
procedure gl_MVertex2f( X, Y : Single ); extdecl;
procedure gl_MVertex2fv( v : Pointer ); extdecl;
...
{$IFDEF WIN32}
  ...
  if ogl_Format = 0 Then
    begin
      ...
      repeat
        ...
        if ( PixelFormat = 0 ) and ( ogl_zDepth = 16 ) Then
          begin
            if not ogl_DoubleBuffer Then
              begin
                break;
              end else
                begin
                  ogl_zDepth       := 32;
                  ogl_DoubleBuffer := FALSE;
                end;
          end else
            DEC( ogl_zDepth, 8 );
      until PixelFormat <> 0;
  ...
  if ( ogl_Format = 0 ) and ( Assigned( wglChoosePixelFormatARB ) ) Then
    begin
      repeat
        ogl_Attr[ 0 ] := WGL_ACCELERATION_ARB;
        ogl_Attr[ 1 ] := WGL_FULL_ACCELERATION_ARB;
        ogl_Attr[ 2 ] := WGL_DRAW_TO_WINDOW_ARB;
        ogl_Attr[ 3 ] := GL_TRUE;
        ogl_Attr[ 4 ] := WGL_SUPPORT_OPENGL_ARB;
        ogl_Attr[ 5 ] := GL_TRUE;
        i := 6;
        if ogl_DoubleBuffer Then
          begin
            ogl_Attr[ i     ] := WGL_DOUBLE_BUFFER_ARB;
            ogl_Attr[ i + 1 ] := GL_TRUE;
            INC( i, 2 );
          end;
        ogl_Attr[ i     ] := WGL_DEPTH_BITS_ARB;
        ogl_Attr[ i + 1 ] := ogl_zDepth;
        INC( i, 2 );
        if ogl_Stencil > 0 Then
          begin
            ogl_Attr[ i     ] := WGL_STENCIL_BITS_ARB;
            ogl_Attr[ i + 1 ] := ogl_Stencil;
            INC( i, 2 );
          end;
        ogl_Attr[ i     ] := WGL_COLOR_BITS_ARB;
        ogl_Attr[ i + 1 ] := scr_BPP;
        ogl_Attr[ i + 2 ] := WGL_ALPHA_BITS_ARB;
        ogl_Attr[ i + 3 ] := 8;
        INC( i, 4 );
        if ogl_FSAA > 0 Then
          begin
            ogl_Attr[ i     ] := WGL_SAMPLE_BUFFERS_ARB;
            ogl_Attr[ i + 1 ] := GL_TRUE;
            ogl_Attr[ i + 2 ] := WGL_SAMPLES_ARB;
            ogl_Attr[ i + 3 ] := ogl_FSAA;
            INC( i, 4 );
          end;
        ogl_Attr[ i ] := 0;
    
        wglChoosePixelFormatARB( wnd_DC, @ogl_Attr, nil, 1, @ogl_Format, @ogl_Formats );
        if ogl_Format = 0 Then
          if ogl_FSAA = 0 Then 
            begin
              break
...
begin
  ...
  if ogl_CanCompress Then
    log_Add( 'GL_ARB_TEXTURE_COMPRESSION: TRUE' )
  else
    log_Add( 'GL_ARB_TEXTURE_COMPRESSION: FALSE' );
  ...
  if Assigned( glBindBufferARB ) Then
    begin
      ...
      ogl_CanVBO := FALSE;
  if ogl_CanVBO Then
    log_Add( 'GL_ARB_VERTEX_BUFFER_OBJECT: TRUE' )
  else
    log_Add( 'GL_ARB_VERTEX_BUFFER_OBJECT: FALSE' );
  ...
  if Assigned( glBindRenderbufferEXT ) Then
    begin
      ...
      ogl_CanFBO := FALSE;
   if ogl_CanFBO Then
     log_Add( 'GL_EXT_FRAMEBUFFER_OBJECT: TRUE' )
   else
     log_Add( 'GL_EXT_FRAMEBUFFER_OBJECT: FALSE' );
  ...
  if ogl_CanPBuffer Then
    log_Add( 'WGL_ARB_PBUFFER: TRUE' )
  else
    log_Add( 'WGL_ARB_PBUFFER: FALSE' );
  {$ENDIF}
...
{$ENDIF}
  if ogl_CanVSync Then
    log_Add( 'Support WaitVSync: TRUE' )
  else
    log_Add( 'Support WaitVSync: FALSE' );
end;
...
begin
  ...
  glLoadIdentity;
  glOrtho( 0, ogl_Width, ogl_Height, 0, 0, 1 );
...
procedure gl_MVertex2fv;
  var
    v2 : array[ 0..1 ] of Single;
begin
  v2[ 0 ] := PSingle( v + 0 )^ * scr_ResCX;
  v2[ 1 ] := PSingle( v + 4 )^ * scr_ResCY;
  glVertex2fv( @v2[ 0 ] );
end;
