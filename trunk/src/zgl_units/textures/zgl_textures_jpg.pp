...
procedure jpg_LoadFromFile( FileName : PChar{String}; var pData : Pointer; var W, H : WORD ); extdecl;
...
{$IFDEF LINUX}
  ...
  if jpgCInfo.out_color_space = JCS_GRAYSCALE Then
    ...
    if jpgCInfo.out_color_space = JCS_RGB Then
      ...
      begin
        u_Error( 'JPG - Unsupported ColorType' );
        goto _exit;
      end;
...
{$ENDIF}
  ...
  pData := AllocMem( jpgData.Width * jpgData.Height * 4 );
