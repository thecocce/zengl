...
procedure vbo_Build( var IBuffer, VBuffer : DWORD; VCount, ICount : DWORD; Indices, Vertices, Normals, TexCoords, MultiTexCoords : Pointer; var Flags : DWORD ); extdecl;
procedure vbo_Free( var IBuffer, VBuffer : DWORD; VCount, ICount : DWORD ); extdecl;
...
procedure vbo_Build;
  var
    ...
    pn, pt  : DWORD;
begin
  if Flags and VBO_USE_MULTITEX3 > 0 Then
    begin
      Flags := Flags or VBO_USE_MULTITEX2;
      Flags := Flags or VBO_USE_MULTITEX1;
    end else
      if Flags and VBO_USE_MULTITEX2 > 0 Then
        Flags := Flags or VBO_USE_MULTITEX1;
  ...
  if ogl_CanVBO Then
    begin
      ...
      size := VCount * 12;
      if Flags and VBO_USE_NORMALS > 0 Then
        begin
          pn := size;
          INC( size, VCount * 12 );
        end;
      if Flags and VBO_USE_TEXTURE > 0 Then
        begin
          pt := size;
          INC( size, VCount * 8 );
          if ogl_MaxTexLevels > 0 Then
            begin
              tTexLevel := Byte( Flags and VBO_USE_MULTITEX1 > 0 ) +
                           Byte( Flags and VBO_USE_MULTITEX2 > 0 ) +
                           Byte( Flags and VBO_USE_MULTITEX3 > 0 );
...
Begin
  if ogl_MaxTexLevels > 0 Then
    tTexLevel := Byte( VBO.Flags and VBO_USE_MULTITEX1 > 0 ) +
                 Byte( VBO.Flags and VBO_USE_MULTITEX2 > 0 ) +
                 Byte( VBO.Flags and VBO_USE_MULTITEX3 > 0 );
  
  if ogl_CanVBO Then
    begin
      PV := 0;
      if VBO.Flags and VBO_USE_NORMALS > 0 Then PN := PV + VBO.VCount * 12;
      if VBO.Flags and VBO_USE_TEXTURE > 0 Then PT := PV + PN + VBO.VCount * 12;
      ...
      begin
        ...
        if VBO.Flags and VBO_USE_NORMALS > 0 Then PN := Ptr( @VBO.Normals[ 0 ] );
        if VBO.Flags and VBO_USE_TEXTURE > 0 Then PT := Ptr( @VBO.TexCoords[ 0 ] );
      end;

  if ( VBO.Flags and VBO_USE_NORMALS > 0 ) and ( glIsEnabled( GL_LIGHTING ) = GL_TRUE ) Then
    begin
      glEnableClientState( GL_NORMAL_ARRAY );
      glNormalPointer( GL_FLOAT, 0, Pointer( PN ) );
    end;
  if VBO.Flags and VBO_USE_TEXTURE > 0 Then
    begin
