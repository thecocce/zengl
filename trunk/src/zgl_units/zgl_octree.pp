...
const
  OCTREE_USE_TEXTURE     = $001;
  OCTREE_USE_MULTITEX1   = $002;
  OCTREE_USE_MULTITEX2   = $004;
  OCTREE_USE_MULTITEX3   = $008;
  OCTREE_USE_NORMALS     = $010;
  OCTREE_BUILD_FNORMALS  = $020;
  OCTREE_BUILD_SNORMALS  = $040; // пока нету :)
  OCTREE_BUILD_VBO       = $080;
  OCTREE_BUILD_PLANES    = $100;
...
begin
  ...
  if tFirstTime Then
    if Octree.Flags and OCTREE_BUILD_VBO > 0 Then
      begin
...
begin
  ...
  if Flags and OCTREE_USE_MULTITEX3 > 0 Then
    begin
      Flags := Flags or OCTREE_USE_MULTITEX2;
      Flags := Flags or OCTREE_USE_MULTITEX1;
    end else
      if Flags and OCTREE_USE_MULTITEX2 > 0 Then
        Flags := Flags or OCTREE_USE_MULTITEX1;
  Octree.Flags := Flags;
  
  tTexLevel := Byte( Octree.Flags and OCTREE_USE_MULTITEX1 > 0 ) +
               Byte( Octree.Flags and OCTREE_USE_MULTITEX2 > 0 ) +
               Byte( Octree.Flags and OCTREE_USE_MULTITEX3 > 0 );
  ...
  if ( Octree.Flags and OCTREE_BUILD_FNORMALS > 0 ) and ( not Assigned( Octree.Normals ) ) Then
    begin
      Octree.Flags := Octree.Flags or OCTREE_USE_NORMALS;
      SetLength( Octree.Normals, Octree.VCount );
      for i := 0 to Octree.FCount - 1 do
        begin
          max := tri_GetNormal( @Octree.Vertices[ Octree.Faces[ i ].vIndex[ 0 ] ],
                                @Octree.Vertices[ Octree.Faces[ i ].vIndex[ 1 ] ],
                                @Octree.Vertices[ Octree.Faces[ i ].vIndex[ 2 ] ] );
          Octree.Normals[ Octree.Faces[ i ].vIndex[ 0 ] ] := max;
          Octree.Normals[ Octree.Faces[ i ].vIndex[ 1 ] ] := max;
          Octree.Normals[ Octree.Faces[ i ].vIndex[ 2 ] ] := max;
        end;
    end;

  if ( Octree.Flags and OCTREE_BUILD_VBO > 0 ) and ( not ogl_CanVBO ) Then
    Octree.Flags := Octree.Flags xor OCTREE_BUILD_VBO;
    
  if Octree.Flags and OCTREE_BUILD_PLANES > 0 Then
    begin
  ...
  if Octree.Flags and OCTREE_USE_TEXTURE > 0 Then
    begin
      SetLength( Octree.TexCoords, Octree.VCount );
      BuildTexCoords( Octree.FCount, Octree.Faces, Octree.VCount, Octree.TexCoords );
    end;

  {if Octree.VCount < 65536 Then
    begin
      Octree.Indices := AllocMem( Octree.FCount * 3 * 2 );
      BuildIndices( Octree.FCount, Octree.Faces, Octree.Indices, 2 );
    end else
      begin
        Octree.Indices := AllocMem( Octree.FCount * 3 * 4 );
        BuildIndices( Octree.FCount, Octree.Faces, Octree.Indices, 4 );
      end;}

  if ( Octree.Flags and OCTREE_BUILD_VBO > 0 ) and ( ogl_CanVBO ) Then tFirstTime := TRUE;
    
  octree_AddNode( Octree, Octree.MainNode, Octree.FCount, Faces );
  SetLength( Faces, 0 );

  if ( Octree.Flags and OCTREE_BUILD_VBO > 0 ) and ( ogl_CanVBO ) Then
    begin
      Octree.VBOFlags := VBO_CLEARDATA;
      if Octree.Flags and OCTREE_USE_NORMALS > 0 Then
        Octree.VBOFlags := Octree.VBOFlags or VBO_USE_NORMALS;
      if Octree.Flags and OCTREE_USE_TEXTURE > 0 Then
        Octree.VBOFlags := Octree.VBOFlags or VBO_USE_TEXTURE;
      if Octree.Flags and OCTREE_USE_MULTITEX1 > 0 Then
        Octree.VBOFlags := Octree.VBOFlags or VBO_USE_MULTITEX1;
      if Octree.Flags and OCTREE_USE_MULTITEX2 > 0 Then
        Octree.VBOFlags := Octree.VBOFlags or VBO_USE_MULTITEX2;
      if Octree.Flags and OCTREE_USE_MULTITEX3 > 0 Then
        Octree.VBOFlags := Octree.VBOFlags or VBO_USE_MULTITEX3;


      vbo_Build( Octree.IBuffer, Octree.VBuffer, Octree.VCount, Octree.ICount, 
                 Octree.Indices, 
                 @Octree.Vertices[ 0 ], @Octree.Normals[ 0 ],
                 @Octree.TexCoords[ 0 ], @Octree.MultiTexCoords[ 0 ],
                 Octree.VBOFlags );
    end;
...
begin
  ...
  if Octree.Flags and OCTREE_BUILD_VBO > 0 Then vbo_Free( Octree.IBuffer, Octree.VBuffer, Octree.VCount, Octree.ICount );
end;
...
begin
  ...
  if Octree.Flags and OCTREE_BUILD_VBO > 0 Then
    begin
      PV := 0;
      if Octree.Flags and OCTREE_USE_NORMALS > 0 Then PN := PV + Octree.VCount * 12;
      if Octree.Flags and OCTREE_USE_TEXTURE > 0 Then PT := PV + PN + Octree.VCount * 12;
      ...
      begin
        ...
        if Octree.Flags and OCTREE_USE_NORMALS > 0 Then PN := Ptr( @Octree.Normals[ 0 ] );
        if Octree.Flags and OCTREE_USE_TEXTURE > 0 Then PT := Ptr( @Octree.TexCoords[ 0 ] );
      end;

  if ( Octree.Flags and OCTREE_USE_NORMALS > 0 ) and ( glIsEnabled( GL_LIGHTING ) = GL_TRUE ) Then
    begin
...
//TODO: оптимизировать(хранить индексы для таких фейсов заранее в памяти а не
