...
// plane
function col3d_PlaneVsSphere( Plane : zglPPlane; Sphere : zglPSphere; Callback : zglTCol3DCallback ) : Boolean; extdecl;
// aabb
...
function col3d_SphereVsNode( Sphere : zglPSphere; Octree : zglPOctree; Node : zglPNode; Callback : zglTCol3DCallback ) : Boolean; extdecl;
...
begin
  ...
  distance := plane_Distance( Plane, Sphere.Position );
	...
	if abs( distance ) < Sphere.Radius Then
  	begin
		  offset := vector_MulV( Plane.Normal, distance );
      point  := vector_Sub( Sphere.Position, offset );

  		if col3d_PointInTri( @point, @Plane.Points[ 0 ], @Plane.Points[ 1 ], @Plane.Points[ 2 ] ) Then
        begin
		      Result := TRUE;
          offset := GetCollisionOffset( @Plane.Normal, Sphere.Radius, distance );
        end else
      ...
      if Result and Assigned( Callback ) Then Callback( @offset );
    end;
end;
...
function col3d_SphereVsSphere;
  var
    ...
    scalar : Single;
begin
	d      := vector_Sub( Sphere1.Position, Sphere2.Position );
	scalar := Sphere1.Radius + Sphere2.Radius;
  d      := vector_Add( d, d );
  // CHECK ME!!!
	Result := ( d.X < ( scalar * scalar ) ) and
            ( d.Y < ( scalar * scalar ) ) and
            ( d.Z < ( scalar * scalar ) );
end;
...
begin
  ...
  if Node.NInside Then
    for i := 0 to 7 do
      if Assigned( Node.SubNodes[ i ] ) Then
        if col3d_SphereVsNode( Sphere, Octree, Node.SubNodes[ i ], Callback ) Then tResult := TRUE;
  INC( colc );
  ...
  if Node.PCount > 0 Then
    for i := 0 to Node.PCount - 1 do
      begin
        inc( colp );
        if col3d_PlaneVsSphere( @Octree.Planes[ Node.Planes[ i ] ], Sphere, Callback ) Then tResult := TRUE;
      end;
  ...
  if Node = Octree.MainNode Then
    writeln( 'END: Node: ', colc, '; Plane: ', colp );
end;
