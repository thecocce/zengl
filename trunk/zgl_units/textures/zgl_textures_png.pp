...
procedure png_LoadFromFile( FileName : PChar{String}; var pData : Pointer; var W, H : WORD ); extdecl;
...
begin
  ...
  if not pngHasIDAT Then
    begin
      u_Error( 'PNG - Image data not found' );
      goto _exit;
    end;

  pData := AllocMem( pngHeader.Width * pngHeader.Height * 4 );
...
begin
  if pngChunk.Size < SizeOf( zglTPNGHeader ) Then
    begin
      u_Error( 'PNG - Invalid header size' );
      pngFail := TRUE;
      exit;
    end;
  ...
  if ( pngHeader.CompressionMethod <> 0 ) Then
    begin
      u_Error( 'PNG - Invalid compression method' );
      pngFail := TRUE;
      exit;
    end;
  ...
  if pngHeader.InterlaceMethod <> 0 Then
    begin
      u_Error( 'PNG - Interlace not supported.' );
      pngFail := TRUE;
      exit;
    end;
...
begin
  ...
  pngRowBuffer[ FALSE ] := AllocMem( pngRowSize + 1 );
  pngRowBuffer[ TRUE ]  := AllocMem( pngRowSize + 1 );
...
begin
  ...
  if not Assigned( CopyP ) Then
    begin
      u_Error( 'PNG - Unsupported ColorType' );
      pngFail := TRUE;
      exit;
    end;
    ...
    begin
      ...
      while avail_out > 0 do
        begin
          if ( pngMem.Position = pngIDATEnd ) and ( avail_out > 0 ) and ( avail_in = 0 ) Then
            begin
              ...
              if IDATHeader <> 'IDAT' Then
                begin
                  u_Error( 'PNG - IDAT chunk expected' );
                  pngFail := TRUE;
                  exit;
                end;
          ...
          if Result < 0 Then
            begin
              u_Error( 'PNG - ZLib error: ' + z_errmsg[ Result ] );
              pngFail := TRUE;
              exit;
            end else
              Result := avail_In;
