...
{$IFDEF LINUX}
  ...
  XF86VidModeGetModeLine( scr_Display, scr_Default, @scr_Desktop.dotclock, PXF86VidModeModeLine( @scr_Desktop + SizeOf( scr_Desktop.dotclock ) ) );
  log_Add( 'Current mode: ' + u_IntToStr( scr_Desktop.hdisplay ) + ' x ' + u_IntToStr( scr_Desktop.vdisplay ) );
  ...
  repeat
    ...
    log_Add( 'glXChooseVisual: zDepth = ' + u_IntToStr( ogl_zDepth ) + '; ' + 
             'colorbit = ' + u_IntToStr( color ) + '; ' + 
             'doublebuffer = ' + u_IntToStr( Byte( ogl_DoubleBuffer ) ) + '; ' +
             'fsaa = ' + u_IntToStr( ogl_FSAA )  );
    ogl_VisualInfo := glXChooseVisual( scr_Display, scr_Default, @ogl_Attr[ 0 ] );
    if ( not Assigned( ogl_VisualInfo ) and ( ogl_zDepth = 1 ) ) Then
      begin
        if ogl_FSAA = 0 Then
          begin
            if ( not ogl_DoubleBuffer ) and ( color = 1 ) Then
              break
            else
              begin
                color            := 1;
...
begin
  ...
  {$ENDIF}
  {$IFDEF WIN32}
  if ( Width >= scr_Desktop.dmPelsWidth ) and ( Height >= scr_Desktop.dmPelsHeight ) Then
    app_FullScreen := TRUE;
  {$ENDIF}
  if app_FullScreen Then
    begin
      ...
      begin
        {$IFDEF LINUX}
        scr_Width  := scr_Desktop.hdisplay;
        scr_Height := scr_Desktop.vdisplay;
        {$ENDIF}
        {$IFDEF WIN32}
  ...
  if ( scr_BPP <> 32 ) and ( scr_BPP <> 16 ) Then
    begin
      u_Warning( 'Wrong screen option, only 32 or 16 bpp support. Set 16 bpp...' );
      scr_BPP := 16;
    end;
...
{$ENDIF}
  if app_FullScreen Then
    ...
    {$ENDIF}
    {$IFDEF WIN32}
    log_Add( 'Set screen options: ' + u_IntToStr( wnd_Width ) + ' x ' + u_IntToStr( wnd_Height ) + ' x ' + u_IntToStr( scr_Desktop.dmBitsPerPel ) + 'bpp windowed' );
    {$ENDIF}
  wnd_Update;
end;
...
begin
  scr_ResCX := ogl_Width  / Width;
  scr_ResCY := ogl_Height / Height;
end;
