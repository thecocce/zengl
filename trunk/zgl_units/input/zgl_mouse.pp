...
function mouse_X : WORD; extdecl;
function mouse_Y : WORD; extdecl;
...
begin
  {$IFDEF LINUX}
  Result := mouse_X() - ( wnd_X + wnd_Width div 2 );
  {$ENDIF}
  {$IFDEF WIN32}
  Result := mouse_X() + wnd_X + wnd_BrdSizeX - wnd_Width div 2;
  {$ENDIF}
end;
...
begin
  {$IFDEF LINUX}
  Result := mouse_Y() - ( wnd_Y + wnd_Height div 2 );
  {$ENDIF}
  {$IFDEF WIN32}
  Result := mouse_Y() + wnd_Y + wnd_BrdSizeY + wnd_CpnSize - wnd_Height div 2;
  {$ENDIF}
end;
...
begin
  ...
  {$ENDIF}
  {$IFDEF WIN32}
  SetCursorPos( wnd_X + wnd_Width div 2, wnd_Y + wnd_Height div 2 );
  {$ENDIF}
end;
