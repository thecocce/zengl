...
function  font_Add : zglPFont; extdecl;
procedure font_Del( Font : zglPFont ); extdecl;

function font_LoadFromFile( Texture, FontInfo : PChar{String} ) : zglPFont; extdecl;

procedure text_Draw( Font : zglPFont; X, Y : Single; _Text : PChar{String}; Alpha : Byte; Color : DWORD; Step, Scale : Single ); extdecl;
function  text_GetWidth( Font : zglPFont; _Text : PChar{String}; Step, Scale : Single ) : Single; extdecl;
...
begin
  ...
  if ID <> ZGL_FONT_INFO Then
    begin
      u_Error( FontInfo + ' - it''s not a ZenGL font info file' );
      zgl_Destroy;
      halt;
    end;
  ...
  for i := 1 to length( Text ) - 1 do
    begin
      ...
      if ( i <> 1 ) and ( Text[ i - 1 ] <> Char( 13 ) ) Then
        X := X + Font.Width[ c2 ] * Scale + ( w - Font.Width[ c2 ] * Scale ) / 2 - ( w - Font.Width[ c ] * Scale ) / ( 2 / ( Byte( i = 1 ) + 1 ) ) + Step * Scale;
