  ...
  repeat
    ...
    if chunkName = 'data' then
      begin
        ...
        if wavHeader.FormatCode = WAV_IMA_ADPCM Then u_Error( 'Unsupported wav format - IMA ADPCM' );
        if wavHeader.FormatCode = WAV_MP3 Then       u_Error( 'Unsupported wav format - MP3' );
      end else
        begin
