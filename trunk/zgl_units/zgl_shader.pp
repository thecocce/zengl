...
function  shader_LoadFromFileARB( FileName : PChar{String}; ShaderType : DWORD ) : DWORD; extdecl;
...
function  shader_LoadFromFile( FileName : PChar{String}; ShaderType : DWORD; Link : Boolean ) : DWORD; extdecl;
...
function  shader_GetUniform( Shader : DWORD; UniformName : PChar{String} ) : Integer; extdecl;
...
begin
  ...
  if Assigned( glGenProgramsARB ) Then
    begin
      ...
      Result := TRUE;
      log_Add( 'Support ARB shaders: TRUE.' );
    end else
      begin
        ogl_CanARB := FALSE;
        u_Error( 'Support ARB shaders: FALSE.' );
      end;
end;
...
begin
  ...
  if s <> '' Then
    begin
      u_Error( 'Shader load error: ' + #13#10 + s );
      Result := 0;
      exit;
    end;
end;
...
begin
  ...
  if Assigned( glCreateShaderObjectARB ) Then
    begin
      ...
      glVertexAttribPointerARB  := wglGetProcAddress( 'glVertexAttribPointerARB' );
      Result := TRUE;
      log_Add( 'Support GLSL shaders: TRUE.' );
    end else
      begin
        ogl_CanGLSL := FALSE;
        u_Error( 'Support GLSL shaders: FALSE.' );
      end;
end;
