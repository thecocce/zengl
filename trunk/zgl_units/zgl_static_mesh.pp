...
function  smesh_LoadFromFile( FileName : PChar; var Mesh : zglPSMesh ) : Boolean; extdecl;
...
begin
  ...
  if not zmf_ReadHeader( M ) Then
    begin
      u_Error( FileName + ' - not a ZenGL Mesh File' );
      mem_Free( M );
      exit;
    end;

  Mesh.Flags  := zmfHeader.Flags;
  ...
  if zmfHeader.Flags and ZMF_USE_NORMALS > 0 Then SetLength( Mesh.Normals, Mesh.VCount );
  if zmfHeader.Flags and ZMF_USE_TEXTURE > 0 Then SetLength( Mesh.TexCoords, Mesh.TCount );
  //SetLength( Mesh.MultiTexCoords, ( zmfHeader.TLayers ) * Mesh.VCount );
...
begin
  if ogl_MaxTexLevels > 0 Then
    tTexLevel := Byte( Mesh.Flags and ZMF_USE_MULTITEX1 > 0 ) +
                 Byte( Mesh.Flags and ZMF_USE_MULTITEX2 > 0 ) +
                 Byte( Mesh.Flags and ZMF_USE_MULTITEX3 > 0 );
                 
  if Mesh.Flags and ZMF_USE_NORMALS > 0 Then
    begin
      glEnableClientState( GL_NORMAL_ARRAY );
      glNormalPointer( GL_FLOAT, 0, @Mesh.Normals[ 0 ] );
    end;
  if Mesh.Flags and ZMF_USE_TEXTURE > 0 Then
    begin
